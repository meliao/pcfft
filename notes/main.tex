...
\begin{align*}
  ...
  &= \boldsymbol{A}^{(\text{add})}_{i,:} \boldsymbol{\mu} - \boldsymbol{A}^{(\text{sub})}_{i,:} \boldsymbol{A} \boldsymbol{\mu}  
\end{align*}
where $\boldsymbol{A}^{(\text{add})}$ maps from source points to target points, and 
$\boldsymbol{A}^{(\text{sub})}$ maps from regular gridpoints to target points. 
Then we can evaluate \eqref{eq:full_sum} as:
\begin{align*}
  u_i &= \sum_{j \in \text{Near}(x_i)} K(x_i - y_j) \mu_j + \sum_{j \not \in \text{Near}(x_i)} K(x_i - y_j) \mu_j \\
  &= \boldsymbol{A}^{(\text{add})}_{i,:} \boldsymbol{\mu} - \boldsymbol{A}^{(\text{sub})}_{i,:} \boldsymbol{A} \boldsymbol{\mu}  + \boldsymbol{K} \boldsymbol{A} \boldsymbol{\mu}
\end{align*}
where $\boldsymbol{K}$ is the kernel matrix from regular grid points to target points.
