...
Let's look at an N-body sum that we are trying to approximate:
\begin{align}
  u_i &= \sum_{j=1}^N K(x_i - y_j) \mu_j \\
...
\begin{align*}
  \sum_{j \not \in \text{Near}(x_i)} K(x_i - y_j) \mu_j \approx \boldsymbol{A}_{i, :} \boldsymbol{\mu}.
\end{align*}
However, $\boldsymbol{A}$ contains all of the interactions, not just far interactions. So we need to compute a matrix $\boldsymbol{B}$ which approximates:
\begin{align*}
  \boldsymbol{B}_{i,:} \boldsymbol{\mu} = \sum_{j \in \text{Near}(x_i)} K(x_i - y_j) \mu_j - \boldsymbol{A}_{i, \text{Near}(i)} \boldsymbol{\mu}
\end{align*}
Steps for computing this matrix:
\begin{itemize}
  \item For each bin $i$, return a list of each bin $j$ where the proxy circle $i$ intersects with the spreading box of $j$.
  \item Compute the exact near-field interactions $\sum_{j \in \text{Near}(x_i)} K(x_i - y_j) \mu_j$.
  \item Compute the contribution of the near-field interacions to $\boldsymbol{A}_i \boldsymbol{\mu}$.
\end{itemize}
\end{document}