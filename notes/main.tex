...
\begin{align*}
  \boldsymbol{B}_{i,:} \boldsymbol{\mu} = \sum_{j \in \text{Near}(x_i)} K(x_i - y_j) \mu_j -  \sum_{k}  K(x_i - z_k)\boldsymbol{A}_{k, \text{Near}(i)} \boldsymbol{\mu}
\end{align*}
...
\begin{itemize}
  ...
  \item Compute the contribution of the near-field interacions to $\sum_{k}  K(x_i - z_k)\boldsymbol{A}_{k, \text{Near}(i)} \boldsymbol{\mu}$.
\end{itemize}
\end{document}
