function [r_sorted, sorted_bin_ids, id_start, sorted_idxes] = bin_pts_2d(r, dx, ngrid, Lbd,  nbin, nspread)
    ...
    % in each dimension.
    % 
    % We want to sort the points into bins which are <nspreadn> 
    ...
    % disp("bin_pts_2d: nspread: " + int2str(nspread))
    ...
    id_x = floor((r(1,:) - Lbd(1) + dx/2) / (nspread * dx));
    % Same for the Y dim.
    id_y = floor((r(2,:) - Lbd(2) + dx/2) / (nspread * dx));
