function [r_srt, binid_srt, ptid_srt, id_start] = bin_pts_2d(r, dx, ...
    Lbd, nbin, nbinpts)
    ...
    % Then ngrid = [9 5]
    % and if we set nbinpts = 3 and nbin = [3 2] we expect 
    ...
    id_x = floor((r(1,:) - Lbd(1) + dx/2) / (nbinpts * dx));
    % Same for the Y dim.
    id_y = floor((r(2,:) - Lbd(2) + dx/2) / (nbinpts * dx));
