...
alpha_ext = alpha_ratio / w^2;
beta_ext = -beta_ratio / w^2;
...
gs_kern = @(s,t) surfwave.capillary.kern(rts_ext,ejs_ext,s,t,'gs_s');
...
u = apply_mat(sigma,ones(S.npts,1),Aspread,Asubtract,kern_hat,isort,ngrid,sparse(S.npts,S.npts));
u = u(isort);
...
subplot(1,2,2)
my_scatter(rsort,[],log10(abs(u2-uplot)/norm(uplot,inf)),'.'); colorbar
% my_scatter(rsort,[],log10(abs(u2)/norm(uplot,inf)),'.'); colorbar
clim([-12,-7])

% u3 = Asubtract*sigma(isort);
% u2 = u2-uplot;
% 
% u2(abs(u2)<1e-4) =0;
% u3(abs(u2)<1e-4) =0;
...
% figure(5)
% subplot(1,2,1)
% my_scatter(rsort,[],real(u-uplot),'.'); colorbar
% clim([-0.1,0.1])
% subplot(1,2,2)
% my_scatter(rsort,[],real(u2-u),'.'); colorbar
% clim([-0.1,0.1])
...
% toc(t1);
% 
% Amat = @(x) x + apply_mat(x,S.wts,Aspread,Asubtract,kern_hat,isort,ngrid,sparse(S.npts,S.npts));
% 
% tic;
