...
for i = 1:n_tol_vals
    ...
    [dx, nspread, nbinpts, proxy_info] = dx_nproxy(k, 3, tol, halfside);
    ...
    K_src_to_target = log_kernel3D(struct('r',source_pts), struct('r',target_pts));
    target_vals = K_src_to_target * src_weights(:);

    proxy_pts = proxy_info.r;
    K_src_to_proxy = log_kernel3D(struct('r',source_pts), struct('r',proxy_pts));
    proxy_vals = K_src_to_proxy * src_weights(:);
    
    % Solve the least squares problem
    rhs = proxy_vals;
    lhs = log_kernel3D(struct('r',box_pts), struct('r',proxy_pts));
    weights_reg = lsqminnorm(lhs, rhs, 1e-14 * norm(lhs, 'fro'));
    
    % Evaluate the approximation
    K_reg_to_target = log_kernel3D(struct('r',box_pts), struct('r',target_pts));
