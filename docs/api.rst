...
To specify points, the user should construct a structure with field ``r`` containing the coordinates of the points. If there are :math:`N` source points in :math:`\mathbb{R}^d` for `d=2,3`, the ``r`` field should be an array with shape ``[d, N]``. The same applies to target points. For example, in 3 dimensions the points might be generated by
