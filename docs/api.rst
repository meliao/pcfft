...
This package expects points to specified using a struct with field ``r`` containing the coordinates of the points. If there are :math:`N` points in :math:`\mathbb{R}^d` for `d=2,3`, the ``r`` field should be an array with shape ``[d, N]``. It should also contain any other source information, such as unit vector ``n`` with shape ``[d, N]``, or a curvature ``kappa`` with shape ``[1,N]``. For example, in 3 dimensions the source and target points might be generated by

.. code:: matlab

   N = 1000; % number of sourcesz
