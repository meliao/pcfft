 ...
 * If there are :math:`N` source points and :math:`M` target points, the kernel function should return an array of shape ``[opdim(1)*M, opdim(2)*N]`` containing the kernel evaluations between each target and source point. If the kernel is vector-valued, then it should be interleaved.
